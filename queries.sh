...
echo "$($PSQL "SELECT teams.name AS winner FROM (    SELECT winner_id, COUNT(*) AS num_wins    FROM games    WHERE year = 2018    GROUP BY winner_id    ORDER BY num_wins DESC    LIMIT 1) AS top_winner JOIN teams ON teams.team_id = top_winner.winner_id")"
